...
public class AlgoTKINC {
    ...
    Map<Integer,Integer> currentMapItemToTWU;
    /**
     * Run the algorithm
     *
    ...
    public void runAlgorithm(String input, String output, boolean eucsPrune, int lower_top_k, int upper_top_k,
        ...
        if (firstTime) {
            ...
            totalDBUtility_OD = 0;
            currentMapItemToTWU = new HashMap<>();
        }
            ...
            while ((thisLine = myInput.readLine()) != null && tid <= lastLine) {
                if (tid >= firstLine) {
                    ...
                    for (int i = 0; i < items.length; i++) {
                        ...
                        Integer currentTwu = currentMapItemToTWU.get(item);
                        if (currentTwu == null) {
                            currentTwu = transactionUtility;
                        }else{
                            currentTwu+=transactionUtility;
                        }
                        currentMapItemToTWU.put(item, currentTwu);
        ...
        Integer maxValue = Collections.max(currentMapItemToTWU.values());
        boolean condition_Rescan = maxValue>(minUtility_upper-minUtility_lower);
